...
class HomePage extends HookWidget {
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    _buildIconButton(
                      ...
                      color: Colors.green,
                      tooltip: 'Ping all',
                      onPressed: () => context.read<AppBloc>().add(AppEventPing()),
                    ),
                  ],
                ),
              ),
      ...
      builder: (context, state) {
        ...
        return Scrollbar(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) => SstpAddressCard(sstp: sstps[index]),
            controller: scrollController1,
            shrinkWrap: true,
          ),
        );
            ...
            if (state is AppStateFiles) {
              ...
              child = ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  Icon icon = const Icon(Icons.download_rounded, color: Colors.green);

                  if (cached.any((e) => e.name == file.name)) {
                    icon = const Icon(Icons.check_circle_rounded, color: Colors.green);
                    final cachedFile = cached.firstWhere((e) => e.name == file.name);
                      ...
                      child: BlocBuilder<AppBloc, AppState>(
                        ...
                        builder: (context, state) {
                          ...
                          return CheckboxListTile(
                            ...
                            subtitle: _buildFileProgress(context, files, index, state),
                          );
                        },
                      ),
                    ),
  ...
  Widget _buildFileProgress(
      BuildContext context, List files, int index, AppState state) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        BlocBuilder<AppBloc, AppState>(
          buildWhen: (_, state) => state is AppStateUniqueProgress && state.key == index,
          builder: (context, state) {
            double value = 0;
            ProgressStatus? progress;
            String text = "Empty";

            if (Storage().sstpFiles.values.any((e) => e.name == files[index].name)) {
              value = 1;
              text = "Done";
            }
        ...
        Row(
          children: [
            IconButton(
              onPressed: () {
                context.read<AppBloc>().add(AppEventDeleteGhFile(files, index));
              },
            ...
            IconButton(
              onPressed: () {
                context.read<AppBloc>().add(AppEventDownloadGhFile(files, index));
              },
              icon: state is AppStateSstpFileChecked ? state.icon : const Icon(Icons.download_rounded, color: Colors.green),
            ),
          ],
        ),
      ],
